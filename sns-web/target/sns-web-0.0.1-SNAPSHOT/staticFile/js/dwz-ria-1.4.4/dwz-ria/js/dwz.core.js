...
var DWZ = {
	...
	status: {ok:200, error:300, timeout:301},
	ui:{sbar:true},
	frag:{}, //page fragment
	_msg:{}, //alert message
	_set:{
	...
	ajaxDone:function(json){
		if (json.status === undefined && json.message === undefined) { // for iframeCallback
			if (alertMsg) return alertMsg.error(json);
			else return alert(json);
		} 
		if(json.status == DWZ.status.error) {
			if(json.message && alertMsg) alertMsg.error(json.message);
		} else if (json.status == DWZ.status.timeout) {
	...
	loadResource:function(resourceKeys,callBack){
		...
		if(resourceKeys.length>0)
		{
			...
			var r=DWZ.resource[key];
			if(document.getElementById(key))
			{
				DWZ.loadResource(resourceKeys,callBack);
			}
			else
			{
			var s;
		...
		if (!_doc.isBind(DWZ.eventType.pageClear)) {
			_doc.bind(DWZ.eventType.pageClear, function(event){
				...
				if (typeof KindEditor!==undefined){
					$("textarea.kindEditor", box).each(function(){
		...
		ajaxUrl: function(op){
			...
			$.ajax({
				...
				success: function(response){
					...
					if (json.status==DWZ.status.timeout){
				...
				status: {
					503: function(xhr, ajaxOptions, thrownError) {
						alert(DWZ.msg("status_503") || thrownError);
					}
				}
			});
		},
